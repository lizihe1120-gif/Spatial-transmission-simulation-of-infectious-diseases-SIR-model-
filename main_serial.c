...
uint32_t normal_rand() {
    normal_seed ^= normal_seed >> 12;
    normal_seed ^= normal_seed << 25;
    normal_seed ^= normal_seed >> 27;
...
//TIME_UPDATE 
void TIME_UPDATE(int *dims, float state[dims[0]][dims[1]], float RECOVER_TIME, int *INFECTED_CELLS, int record[14000][2], int *record_number, int *ONE_INFECTED_CELLS){
...
void TIME_UPDATE_FAST(int *dims, float state[dims[0]][dims[1]], float RECOVER_TIME, int *INFECTED_CELLS, int record[14000][2], int *record_number, int *ONE_INFECTED_CELLS){
    //#pragma omp parallel for schedule(guided) num_threads(64)
    for(int i =0; i < dims[0]; i++){
        for(int j =0; j < dims[1]; j++){
            if(RECOVER_TIME<state[i][j]&&state[i][j]<2){
...
//ONE_TIME
void RUN(int *dims, float state[dims[0]][dims[1]], float *rate, float RECOVER_TIME, int R, int *INFECTED_CELLS, int *ONE_INFECTED_CELLS){
    ...
    TIME_UPDATE(dims, state, RECOVER_TIME,INFECTED_CELLS, record, &record_number, ONE_INFECTED_CELLS);
    for(; i == true;){
        i = INFECT_UPDATE_FAST(dims, state, rate, RECOVER_TIME, R, record, record_number);
        memset(record, 0, sizeof(int)*14000*2);
        record_number =0; 
        TIME_UPDATE_FAST(dims, state, RECOVER_TIME,INFECTED_CELLS, record, &record_number, ONE_INFECTED_CELLS);
        if (i==false)
        {
            break;
        }
...
int main (int i, char *basical_data[]){
    ...
    float AVERAGE_CELLS_INFECTED = 1;
    int INFECTED_CELLS = 0;
    int ONE_INFECTED_CELLS =0;
    int MAX_INFECTED =0;
    float caculate[MAX_RUNS];
    float STD_DEV =0;
    ...
    for(int a =0; a<MAX_RUNS; a++){
        ONE_INFECTED_CELLS =1;
        float state[dims[0]][dims[1]];
        memset(state, 0, sizeof(float) * dims[0] * dims[1]);
        //ZERO
        state[normal_rand() % dims[0]][normal_rand() % dims[1]] = 1.00;
        INFECTED_CELLS++;
        RUN(dims, state, rate, RECOVER_TIME, R,&INFECTED_CELLS, &ONE_INFECTED_CELLS);
        caculate[a]=ONE_INFECTED_CELLS;
        if(MAX_INFECTED < ONE_INFECTED_CELLS){
            MAX_INFECTED = ONE_INFECTED_CELLS;
        }
    }
    ...
    {
        caculate[i] = caculate[i] - AVERAGE_CELLS_INFECTED;
        caculate[i] = caculate[i]*caculate[i];
        STD_DEV = STD_DEV + caculate[i];
    }
    STD_DEV = STD_DEV/MAX_RUNS;
    STD_DEV = sqrt(STD_DEV);
    // STD_DEV = 0.84 *(sqrt(STD_DEV))- (dims[1]/2.44);
    // STD_DEV = STD_DEV - (1.5*(R-2));
